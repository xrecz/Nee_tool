...
SCAN_PROFILES: dict[str, ScanProfile] = {
    "quick": ScanProfile(
        name="Quick Scan",
        description="Schnelle Übersicht (~15 Min). Subdomain, Ports, Web-Check, Header, WAF-Erkennung.",
        scanners=[
            "subdomain", "portscan", "web_discovery",
            "security_headers", "ssl_check", "waf_detect",
        ],
        top_ports=100,
    ...
    "standard": ScanProfile(
        ...
        description="Vollständiger Scan (~1-2 Std). Alle Scanner inkl. API-Erkennung.",
        scanners=[
            "subdomain", "portscan", "web_discovery", "security_headers",
            "ssl_check", "tech_detect", "waf_detect", "crawler",
            "nuclei", "dir_bruteforce", "api_discovery", "cve_enrichment",
        ],
        top_ports=1000,
    ...
    "deep": ScanProfile(
        name="Deep Scan",
        description="Tiefenanalyse (~4+ Std). Maximale Abdeckung inkl. Default-Credentials.",
        scanners=[
            "subdomain", "portscan", "web_discovery", "security_headers",
            "ssl_check", "tech_detect", "waf_detect", "crawler",
            "nuclei", "dir_bruteforce", "api_discovery",
            "default_creds", "cve_enrichment",
        ],
        top_ports=5000,
    ...
    "compliance": ScanProfile(
        ...
        description="Fokus auf Konfigurations- und Header-Compliance, API-Sicherheit (~30 Min).",
        scanners=[
            "subdomain", "web_discovery", "security_headers", "ssl_check",
            "tech_detect", "waf_detect", "api_discovery", "cve_enrichment",
        ],
        top_ports=100,
    ...
    "recon": ScanProfile(
        name="Recon Only",
        description="Reconnaissance mit WAF-/API-/Default-Credential-Erkennung (~30 Min).",
        scanners=[
            "subdomain", "portscan", "web_discovery", "tech_detect",
            "waf_detect", "api_discovery", "default_creds",
        ],
        top_ports=1000,
        timeout_per_scanner=300,
    ),
